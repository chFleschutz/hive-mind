...
void UPerlinNoiseGenerator::initialize(int32 tileGridSize, int32 noiseCellSize, uint32 seed)
{
	Seed = seed;
...
float UPerlinNoiseGenerator::perlinNoise2D(int32 posX, int32 posY)
{
...
FVector2D UPerlinNoiseGenerator::randomVec()
{
	auto randAngle = FMath::FRand() * 2.0f * UE_PI;
	return FVector2D(FMath::Cos(randAngle), FMath::Sin(randAngle));
}

float UPerlinNoiseGenerator::CornerGradientDotProduct(int32 cornerPosX, int32 cornerPosY, float posX, float posY)
{
...
FVector2D UPerlinNoiseGenerator::noiseVec(uint32 x, uint32 y)
{
